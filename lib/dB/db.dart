...
class DatabaseSQL {
  ...
  Future _onCreate(Database db, int version) async {
    ...
    Future<List<Map<String, dynamic>>> get_shoping_cart_item({required String soping_cart_id, required String product_id}) async {
      return await _db.rawQuery('SELECT * FROM shoping_cart_items where soping_cart_id=$soping_cart_id and product_id=$product_id');
    }
    
    Future<List<Map<String, dynamic>>> get_shoping_cart_items({required String soping_cart_id}) async {
      return await _db.rawQuery('SELECT * FROM shoping_cart_items where soping_cart_id=$soping_cart_id');
    }
    Future<int> product_count_increment({required String item_id, required int count}) async {
      return await _db.rawUpdate('UPDATE shoping_cart_items SET count=$count where id=$item_id');
    }
    Future<int> delete_item({required String item_id}) async {
      return await _db.rawDelete('DELETE FROM shoping_cart_items where id=$item_id');
    }

    Future<int> delete_shoping_cart_items({required int shoping_cart_id}) async {
      return await _db.rawDelete('DELETE FROM shoping_cart_items where soping_cart_id=$shoping_cart_id');
    }
